...
import { Bean, GlobalSettings, FeeSettings, BlendRecipe, InventoryItem, InventoryOperationLog, SetProduct } from '../types';
...
export const StorageProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        setInventory([]);
        setInventoryHistory([]); // History is cleared on load, will be session based or fetched
        setSetProduct(getStoredValue(STORAGE_KEYS.SET_PRODUCT, initialSetProduct));
        setIsHydrated(true);
    }, []);

    // 2. Supabase Sync & Credit Initialization
    useEffect(() => {
        const syncWithSupabase = async () => {
            if (!user || !isHydrated) {
                ...
                setActiveBeanId('bean-1'); // Reset active bean
                setBlendRecipe(initialRecipe);
                setSetProduct(initialSetProduct);
                // Optional: Keep settings? Or reset? User said "working content" (sagyou-chuu no naiyou).
                // Usually settings might be personal preference, but specific project settings should reset.
                // Global/Fee settings are kinda "project" settings here. Let's reset them for safety.
                setGlobalSettings(initialGlobalSettings);
                setFeeSettings(initialFeeSettings);
                setInventoryHistory([]); // Clear history
            ...
            try {
                ...
                // A. Initialize User & Get Credits (RPC)
                try {
                    const { data: profile, error: profileError } = await supabase.rpc('initialize_user_with_credits');
                    if (profile) {
                    ...
                    console.error('RPC Error (Profile):', rpcError);
                }

                // B. Fetch remote inventory data
                const { data: remoteInventory, error: inventoryError } = await supabase
                    .from('inventory')
                    .select('*')
                ...
                if (historyError) {
                    ...
                    setInventoryHistory(remoteHistory.map((h: any) => ({
                        ...
                        relatedLogIds: [] // Simple mapping for now
                    })));
                }

                // Sync Logic:
                // If remote is empty AND we have local data, migrate local -> remote
                if ((!remoteInventory || remoteInventory.length === 0) && inventory.length > 0) {
                    console.log('Migrating local data to Supabase...', inventory);
                    const migrationPromises = inventory.map(item =>
